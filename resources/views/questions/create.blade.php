...
<div class="container mt-5">
    ...
    <form method="POST" action="{{ route('questions.store') }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="option{{ $i }}" id="option{{ $i }}" class="form-control" required value="{{ old('option'.$i) }}">
        </div>
        @endfor

        <div class="mb-3">
            <label for="correct_option" class="form-label">Correct Option (1-4)</label>
            <input type="number" name="correct_option" id="correct_option" class="form-control" min="1" max="4" required value="{{ old('correct_option') }}">
        </div>
